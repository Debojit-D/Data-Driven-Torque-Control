...
# Rich multi-sine reference: many sinusoidal components per joint.
# The per-joint total excursion stays near these values, but each joint now uses
# multiple frequencies instead of one sine wave.
JOINT_TOTAL_EXCURSION_DEG = np.array([8.0, 8.0, 6.0, 6.0, 4.0, 4.0, 4.0], dtype=np.float64)
BASE_MULTISINE_FREQUENCIES_HZ = np.array(
    [0.11, 0.17, 0.23, 0.31, 0.41, 0.53, 0.67, 0.83, 1.01, 1.21, 1.43, 1.67],
    dtype=np.float64,
)
JOINT_FREQUENCY_SCALE = np.array([1.00, 1.03, 1.07, 1.11, 0.97, 1.14, 0.93], dtype=np.float64)
BASE_MULTISINE_PHASE_DEG = np.linspace(0.0, 330.0, BASE_MULTISINE_FREQUENCIES_HZ.size, dtype=np.float64)
JOINT_PHASE_OFFSET_DEG = np.array([0.0, 23.0, 47.0, 71.0, 109.0, 149.0, 197.0], dtype=np.float64)

MULTISINE_COMPONENT_AMPLITUDES_DEG = (
    JOINT_TOTAL_EXCURSION_DEG[:, None] / BASE_MULTISINE_FREQUENCIES_HZ.size
)
MULTISINE_COMPONENT_FREQUENCIES_HZ = (
    JOINT_FREQUENCY_SCALE[:, None] * BASE_MULTISINE_FREQUENCIES_HZ[None, :]
)
MULTISINE_COMPONENT_PHASES_DEG = np.mod(
    JOINT_PHASE_OFFSET_DEG[:, None] + BASE_MULTISINE_PHASE_DEG[None, :],
    360.0,
)
...
# Injected Gaussian torque noise for richer input excitation.
TORQUE_NOISE_STD = np.array([0.40, 0.40, 0.35, 0.30, 0.10, 0.10, 0.08], dtype=np.float64)
TORQUE_NOISE_SEED = 7
...
class RichMultiSineJointPlanner:
    """
    Simultaneous rich multisine excitation around q_home:
        q_des(t) = q_home + q_center_offset + e(t) * sum_k A_k sin(w_k t + phi_k)
    ...
    def __init__(self, q_home, amplitudes_deg, frequencies_hz, phases_deg, T, T_ramp,
        ...
        self.A = np.deg2rad(np.asarray(amplitudes_deg, dtype=np.float64).reshape(7, -1))
        self.f = np.asarray(frequencies_hz, dtype=np.float64).reshape(7, -1)
        self.w = 2.0 * np.pi * self.f
        self.phi = np.deg2rad(np.asarray(phases_deg, dtype=np.float64).reshape(7, -1))
        self.T = float(T)
        self.T_ramp = float(T_ramp)

        if not (self.A.shape == self.f.shape == self.phi.shape):
            raise ValueError("Amplitude, frequency, and phase arrays must share shape (7, K)")
    ...
    def evaluate(self, t):
        ...
        q_offset = np.sum(e * self.A * s, axis=1)
        qd_offset = np.sum(self.A * (de * s + e * self.w * c), axis=1)
        qdd_offset = np.sum(
            self.A * (dde * s + 2.0 * de * self.w * c - e * (self.w**2) * s),
            axis=1,
        )
...
def apply_joint_pd_control(model, data, q_des, qd_des, rng):
    """
    PD torque control with gravity/bias compensation:
        tau = qfrc_bias + Kp(q_des - q) + Kd(qd_des - qd) + noise
    """
    data.ctrl[:] = 0.0
    ...
    tau_noise_all = rng.normal(0.0, TORQUE_NOISE_STD, size=7)
    ...
    for aid in range(7):
        ...
        tau_cmd = tau_bias + tau_kp + tau_kd + tau_noise_all[aid]
    ...
    return tau_bias_all, tau_kp_all, tau_kd_all, tau_noise_all, tau_cmd_all
...
def plot_joint_tracking(time_hist, q_hist, q_des_hist):
    ...
    fig.suptitle("Rich Multi-Sine Excitation: Position Tracking")
    fig.tight_layout()
    return fig
...
def plot_joint_velocity_tracking(time_hist, qd_hist, qd_des_hist):
    ...
    axes[7].axis("off")
    axes[6].set_xlabel("Time (s)")
    fig.suptitle("Rich Multi-Sine Excitation: Velocity Tracking")
    fig.tight_layout()
    return fig


def plot_torque_components(time_hist, tau_cmd_hist, tau_bias_hist, tau_kp_hist, tau_kd_hist, tau_noise_hist):
    fig, axes = plt.subplots(4, 2, figsize=(14, 12), sharex=True)
    axes = axes.flatten()
    ...
    for j in range(7):
        ...
        axes[j].plot(time_hist, tau_noise_hist[:, j], label="Noise term", linewidth=1.0, alpha=0.9)
    ...
    axes[7].axis("off")
    axes[6].set_xlabel("Time (s)")
    fig.suptitle("Rich Multi-Sine Excitation: Torque Breakdown")
    fig.tight_layout()
    return fig
...
def plot_reference_spectrum_info():
    ...
    component_indices = np.arange(1, MULTISINE_COMPONENT_AMPLITUDES_DEG.shape[1] + 1)
    joint_ticks = np.arange(7)
    joint_labels = [f"q{idx}" for idx in range(1, 8)]

    amplitude_map = axes[0].imshow(MULTISINE_COMPONENT_AMPLITUDES_DEG, aspect="auto", cmap="viridis")
    axes[0].set_ylabel("Joint")
    axes[0].set_yticks(joint_ticks)
    axes[0].set_yticklabels(joint_labels)
    axes[0].set_title("Component Amplitudes (deg)")
    fig.colorbar(amplitude_map, ax=axes[0], label="deg")

    frequency_map = axes[1].imshow(MULTISINE_COMPONENT_FREQUENCIES_HZ, aspect="auto", cmap="plasma")
    axes[1].set_ylabel("Joint")
    axes[1].set_yticks(joint_ticks)
    axes[1].set_yticklabels(joint_labels)
    axes[1].set_title("Component Frequencies (Hz)")
    fig.colorbar(frequency_map, ax=axes[1], label="Hz")

    phase_map = axes[2].imshow(MULTISINE_COMPONENT_PHASES_DEG, aspect="auto", cmap="twilight")
    axes[2].set_ylabel("Joint")
    axes[2].set_yticks(joint_ticks)
    axes[2].set_yticklabels(joint_labels)
    axes[2].set_xticks(component_indices - 1)
    axes[2].set_xticklabels(component_indices)
    axes[2].set_xlabel("Component index")
    axes[2].set_title("Component Phases (deg)")
    fig.colorbar(phase_map, ax=axes[2], label="deg")
...
def main():
    ...
    rng = np.random.default_rng(TORQUE_NOISE_SEED)

    planner = RichMultiSineJointPlanner(
        q_home=q_home,
        amplitudes_deg=MULTISINE_COMPONENT_AMPLITUDES_DEG,
        frequencies_hz=MULTISINE_COMPONENT_FREQUENCIES_HZ,
        phases_deg=MULTISINE_COMPONENT_PHASES_DEG,
    ...
    print("Total joint excursion targets (deg):", JOINT_TOTAL_EXCURSION_DEG)
    print("Multi-sine component amplitudes shape:", MULTISINE_COMPONENT_AMPLITUDES_DEG.shape)
    print("Multi-sine component frequencies shape:", MULTISINE_COMPONENT_FREQUENCIES_HZ.shape)
    print("Multi-sine component phases shape:", MULTISINE_COMPONENT_PHASES_DEG.shape)
    ...
    print("Torque noise std (Nm):", TORQUE_NOISE_STD)
    print("Torque noise seed:", TORQUE_NOISE_SEED)
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running() and data.time <= EXCITATION_DURATION:
            ...
            tau_bias, tau_kp, tau_kd, tau_noise, tau_cmd = apply_joint_pd_control(
                model, data, q_des, qd_des, rng
            )
            ...
            tau_noise_hist.append(tau_noise.copy())
    ...
    tau_noise_hist = np.asarray(tau_noise_hist, dtype=np.float64)
    ...
    output_dir = create_output_dir("analysisV3")
    gpe_result = {
        "checked": False,
    ...
    if SAVE_DATA:
        ...
        np.savez(
            ...
            tau_noise_hist=tau_noise_hist,
            q_home=q_home,
            joint_total_excursion_deg=JOINT_TOTAL_EXCURSION_DEG,
            multisine_component_amplitudes_deg=MULTISINE_COMPONENT_AMPLITUDES_DEG,
            multisine_component_frequencies_hz=MULTISINE_COMPONENT_FREQUENCIES_HZ,
            multisine_component_phases_deg=MULTISINE_COMPONENT_PHASES_DEG,
            center_offset_deg=JOINT_CENTER_OFFSET_DEG,
            ...
            torque_noise_seed=TORQUE_NOISE_SEED,
        )
        print(f"Saved dataset to: {os.path.abspath(save_path)}")

    metadata = {
        "script": "analysisV3.py",
        "output_dir": output_dir,
        "xml_file_path": XML_FILE_PATH,
        "settings": {
            "excitation_duration": float(EXCITATION_DURATION),
            "ramp_time": float(RAMP_TIME),
            "joint_total_excursion_deg": JOINT_TOTAL_EXCURSION_DEG.tolist(),
            "multisine_num_components": int(MULTISINE_COMPONENT_AMPLITUDES_DEG.shape[1]),
            "multisine_component_amplitudes_deg": MULTISINE_COMPONENT_AMPLITUDES_DEG.tolist(),
            "multisine_component_frequencies_hz": MULTISINE_COMPONENT_FREQUENCIES_HZ.tolist(),
            "multisine_component_phases_deg": MULTISINE_COMPONENT_PHASES_DEG.tolist(),
            ...
            "torque_noise_std": TORQUE_NOISE_STD.tolist(),
            "torque_noise_seed": int(TORQUE_NOISE_SEED),
    ...
    save_figure(
        plot_torque_components(
            time_hist, tau_cmd_hist, tau_bias_hist, tau_kp_hist, tau_kd_hist, tau_noise_hist
        ),
        output_dir,
